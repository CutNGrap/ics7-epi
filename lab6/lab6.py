...
def graphs(): 
    ...
    sced_values = [1.0, 1.04, 1.1] 
    mode = 'inter'

    for sced in sced_values:
        ...
        rely_result = GraphExperiment(eaf, mode, [0.75, 0.86, 1.0, 1.15, 1.4], 'RELY')
        eaf = get_standart_eaf()
        eaf['SCED'] = sced
        data_result = GraphExperiment(eaf, mode, [0.94, 1.0, 1.08, 1.16], 'DATA')
        eaf = get_standart_eaf()
        eaf['SCED'] = sced
        cplx_result = GraphExperiment(eaf, mode, [0.7, 0.85, 1.0, 1.15, 1.3], 'CPLX')

        pyplot.subplot(121)
        pyplot.title("Трудозатраты (SCED = " + str(sced) + ")")
        pyplot.plot(['очень низкий', 'низкий', 'номинальный', 'высокий', 'очень высокий'], rely_result[0], label='Надежность (RELY)')
        pyplot.plot(['низкий', 'номинальный', 'высокий', 'очень высокий'], data_result[0], label='Размер БД (DATA)')
        pyplot.plot(['очень низкий', 'низкий', 'номинальный', 'высокий', 'очень высокий'], cplx_result[0], label='Сложность (CPLX)')
        pyplot.ylabel("Трудозатраты, в человекомесяцах")
        pyplot.xlabel("Уровень фактора")
        pyplot.legend()

        pyplot.subplot(122)
        pyplot.title("Время (SCED = " + str(sced) + ")")
        pyplot.plot(['очень низкий', 'низкий', 'номинальный', 'высокий', 'очень высокий'], rely_result[1], label='Надежность (RELY)')
        pyplot.plot(['низкий', 'номинальный', 'высокий', 'очень высокий'], data_result[1], label='Размер БД (DATA)')
        pyplot.plot(['очень низкий', 'низкий', 'номинальный', 'высокий', 'очень высокий'], cplx_result[1], label='Сложность (CPLX)')
        pyplot.ylabel("Время выполнения, в месяцах")
        pyplot.xlabel("Уровень фактора")
        pyplot.legend()
        pyplot.show()
